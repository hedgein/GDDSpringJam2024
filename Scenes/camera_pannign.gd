...
func _ready():
	...
	limit_right = world_size.x / 5
	limit_bottom = world_size.y / 5
...
func _process(delta):
	...
	if x > floor(limit_right * 0.5):
		x = floor(limit_right *0.5)
	if y > floor(limit_bottom * 0.5):
		y = floor(limit_bottom *0.5)
